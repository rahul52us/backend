...
export const StudentBodySchema = Joi.object({
  ...
  startYear: Joi.date().greater('now').allow(null).optional().messages({
  ...
  sibling: Joi.number(),
  medium: Joi.number().valid(0, 1, 2),
  standard: Joi.number().valid(
    1,
    2,
    3,
    4,
...
export const UpdateStudentBodySchema = Joi.object({
...
export const getStudentSchema = Joi.object({
  ...
  standard: Joi.number().min(0).max(16),
  startYear: Joi.date().allow(null).optional(),
  endYear: Joi.date()
    .greater(Joi.ref('startYear'))
    .min(Joi.ref('startYear'))
    .allow(null)
...
export const getSingleStudentSchema = Joi.object({
